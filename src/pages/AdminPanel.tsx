 ...
 * Manages plan definitions and per-plan module access.
 */
...
function ModuleAccessGrid({
  ...
  useEffect(() => {
    setLocalAccess(moduleAccess);
  }, [moduleAccess]);

  const getAccess = (planTier: string, module: string) => {
    return localAccess.find((a) => a.plan_tier === planTier && a.module === module);
  };

  const updateAccess = (planTier: string, module: string, newAccess: string) => {
    setLocalAccess((prev) =>
      prev.map((a) =>
        a.plan_tier === planTier && a.module === module
          ? { ...a, access: newAccess as any }
          : a
      )
    );
  };
  ...
  const cellKey = (plan: string, mod: string) => `${plan}:${mod}`;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Control what each plan can access. Click a cell to edit.
          </p>
        </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              {plans.map((p) => (
                <th key={p.plan_tier} className="text-center p-2.5 font-medium text-muted-foreground">
                  {p.display_name}
                </th>
              ))}
            </tr>
            ...
            {MODULE_LIST.map((mod) => (
              <tr key={mod.key} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                ...
                {plans.map((plan) => {
                  ...
                  const key = cellKey(plan.plan_tier, mod.key);
                  ...
                  return (
                    <td key={plan.plan_tier} className="p-2 text-center">
                      {isEditing ? (
                        <div className="space-y-1.5">
                          <Select
                            value={access?.access || "full"}
                            onValueChange={(v) => updateAccess(plan.plan_tier, mod.key, v)}
                          >
                          ...
                          {(access?.access === "locked") && (
                            <Input
                              ...
                              value={access?.locked_message || ""}
                              onChange={(e) => updateMessage(plan.plan_tier, mod.key, "locked_message", e.target.value)}
                              className="h-6 text-[10px]"
                            />
                          )}
                          {(access?.access === "limited") && (
                            <Input
                              placeholder="Limited note..."
                              value={access?.limited_note || ""}
                              onChange={(e) => updateMessage(plan.plan_tier, mod.key, "limited_note", e.target.value)}
                              className="h-6 text-[10px]"
                            />
                          )}
                          <Button
      ...
      <div className="space-y-6 max-w-5xl">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage plans, pricing, and feature visibility across the platform.
          </p>
        </div>
        ...
        <Tabs defaultValue="plans" className="space-y-4">
          ...
          <TabsContent value="plans" className="space-y-4">
            <p className="text-xs text-muted-foreground">
              Edit plan names, pricing, features, and limits. Changes apply immediately to all users.
            </p>
